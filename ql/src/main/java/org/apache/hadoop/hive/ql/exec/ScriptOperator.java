...
public class ScriptOperator extends Operator<ScriptDesc> implements
    ...
    private static final String SCRIPT_EXTENSION_FACTORY_CLASS = "com.github.dryangkun.hive.HxScriptExtensionFactory";

    public interface ScriptExtensionFactory {
        void initialize(Configuration conf);
        ScriptExtension createExtension(String prog);
    }

    public interface ScriptExtension {
        void initializeOp(ScriptOperator operator, String[] cmdArgs) throws Exception;
        void process(Object row, int tag) throws Exception;
        void close(boolean abort) throws Exception;
    }
    ...
    public static enum Counter {
        DESERIALIZE_ERRORS, SERIALIZE_ERRORS, EXTENSION_ERRORS
    }
    ...
    private final transient LongWritable extension_error_count = new LongWritable();
    ...
    private transient ScriptExtension scriptExtension;
    ...
    protected Collection<Future<?>> initializeOp(Configuration hconf) throws HiveException {
        ...
        statsMap.put(Counter.EXTENSION_ERRORS.toString(), extension_error_count);
        ...
        try {
            ...
            try {
                Class<?> extensionFactoryClass = Class.forName(SCRIPT_EXTENSION_FACTORY_CLASS);
                ScriptExtensionFactory extensionFactory =
                        (ScriptExtensionFactory) extensionFactoryClass.newInstance();
                extensionFactory.initialize(hconf);

                String prog = splitArgs(conf.getScriptCmd())[0];
                scriptExtension = extensionFactory.createExtension(prog);
            } catch (Exception e) {
                LOG.warn("create extension fail", e);
            }
            if (scriptExtension != null) {
                LOG.info("script extension - " + scriptExtension.getClass());
                StructObjectInspector soi = (StructObjectInspector) outputObjInspector;
                List<? extends StructField> structFields = soi.getAllStructFieldRefs();

                List<String> structFieldNames = new ArrayList<>(structFields.size());
                List<ObjectInspector> structFieldObjectInspectors
                        = new ArrayList<>(structFields.size());
                List<String> structComments = new ArrayList<>(structFields.size());
                for (StructField structField : structFields) {
                    structFieldNames.add(structField.getFieldID(),
                    ...
                    structComments.add(structField.getFieldID(),
                            structField.getFieldComment());
                }
                outputObjInspector = ObjectInspectorFactory.getStandardStructObjectInspector(
                        structFieldNames, structFieldObjectInspectors, structComments);
            }
    ...
    public Configuration getConfigurationForExtension() {
        return hconf;
    }

    public Map<String, LongWritable> getStatsMapForExtension() {
        return statsMap;
    }

    public void forwardForExtension(Object row, ObjectInspector rowInspector) throws HiveException {
    ...
    public void process(Object row, int tag) throws HiveException {
        ...
        if (firstRow) {
            ...
            try {
                ...
                if (scriptExtension != null) {
                    scriptExtension.initializeOp(this, cmdArgs);
                } else {
        ...
        if (scriptExtension != null) {
            try {
                scriptExtension.process(row, tag);
            } catch (Exception e) {
                LOG.error("Error in extension process: " + e.getMessage());
                scriptError = e;
                extension_error_count.set(extension_error_count.get() + 1);
    ...
    public void close(boolean abort) throws HiveException {
        if (scriptExtension != null) {
            ...
            try {
                scriptExtension.close(abort);
            } catch (Exception e) {
                LOG.error("close extension fail", e);
            }
            super.close(abort);
            return;
        }
